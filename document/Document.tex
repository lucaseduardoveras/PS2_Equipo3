...
El modelo con mejor rendimiento de los expuestos anteriormente fue el Random Forest, presentado en dos versiones: uno calibrado con validación cruzada (5-fold), con una búsqueda del número de variables de un valor \textit{m} entre 6, 8, 9 y 10; y con un entrenamiento de 1,000 árboles, con la selección de las variables más importantes usando gini. El otro fue un Random Forest tipo Bagging, es decir, el número de variables igual al total de variables disponibles (26), 500 árboles y el ajuste de importancia con permutación. Notese que con ambos modelos intentamos distintas estrategias para la selección de hiperparámetros. En el primero fijamos el número mínimo de observaciones en los nodos a uno, para asegurar árboles muy complejos y así obtener una predicción con menos sesgo. Por el contrario, con Bagging si permitimos al modelo elegir un valor y añadimos una manera adicional de manejar el desbalance: elegir 

A continuación se presenta la comparación entre este modelo y los cinco modelos mejor calibrados por debajo de él.\\

\textbf{RF vs. Lineal.} A pesar de que en este modelo fueron seleccionadas interacciones no lineales entre las variables de género y educación, es claro que por el volumen de predictores existentes en la base de datos una agregación manual de las interacciones es insuficiente. El Random Forest lo hace automáticamente. Además, el Random Forest estuvo calibrado a F1 que se alinea con los parámetros de la competencia de Kaggle, mientras que el modelo lineal permanece limitado por su frontera casi lineal.\\

\textbf{RF vs. Logit.} El modelo de Random Forest superó al modelo Logit principalmente porque la relación entre las variables explicativas y la pobreza, como se ha expuesto antes, no es lineal u homogénea. El modelo Logit impone una frontera de decisión lineal, lo que limita su capacidad de capturar interacciones complejas que el RF hace profundamente. En este sentido, el RF construye múltiples árboles de decisión que dividen el espacio en subregiones que permiten modelar umbrales más flexibles. Por último, no seleccionar los hiperparámetros impide que utilizando el Logit se decida la capacidad de ajuste del modelo. \\

\textbf{RF vs. Elastic Net con LASSO.} Si bien el modelo EN introduce regularización a través de los parámetros \(\lambda\) y \(\alpha\)\\ -que justamente permite reducir la varianza y evita el sobreajuste-, su capacidad de ajuste sigue siendo lineal en los predictores, en contraposición a la naturaleza no lineal del RF. Además, mientras que la penalización LASSO de Elastic Net controla la magnitud de los coeficientes, y por ende facilitando interpretabilidad, también puede reducir la contribución a la predicción de las variables altamente correlacionadas como las demográficas y laborales.\\ 

\textbf{RF vs. Naive Bayes con downsampling.} El Naive Bayes con downsampling mejoró la sensibilidad respecto a su versión sin rebalanceo, de forma que el clasificador dejó de sesgarse hacia la categoría de "no pobre", pero su rendimiento general fue menor al de los RF. Esto se debe principalmente a que el NB asume independencia condicional entre los predictores y modela cada variable por separado, lo que es impertinente para este ejercicio, pues las variables de hogar y laborales están altamente correlacionadas. Además, el downsampling reduce la información de la clase mayoritaria y puede aumentar la varianza del estimador, haciendo que el ajuste dependa más del umbral óptimo.\\

\textbf{RF vs. Gradient Boosting.} El Gradient Boosting capturó no linealidades e interacciones mediante la suma secuencial de árboles débiles, pero fue insuficiente en comparación a los modelos RF por varias razones. La primera es que la proporción de \textit{shrinkage} (de entre 0.01 y 0.001) y número de árboles (máximo de 150) da cuenta de un modelo subentrenado, lo que seguro implicó una limitación en la capacidad del modelo de refinar los errores de etapas previas. Además, en el modelo de GB no se usaron pesos de clase, lo que llevó a favorecer a la clase maoritaria. Por último, el GB es más sensible a la especificación de hiperparámetros, que, sin las iteraciones suficientes, terminan por no generalizar bien. En contraposición a esto último, los modelos de RF combinaron muchos árboles y un submuestreo de variables en cada división, lo que redujo la varianza.\\
